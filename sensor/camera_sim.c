...
{
    char *json = malloc(70);
    sprintf(json, "{\"req_type\": \"POST\", \"is_hallway\": %s, \"node_type\": \"camera\", \"id\": %d, \"people_count\": %d, \"x\": %d, \"y\": %d, \"longitude\": %d, \"latitude\": %d, \"edge1\": %d, \"edge2\": %d}",
                rand() % 2 == 2 ? "true" : "false", ce.id, ce.people_count, ce.dimension.x, ce.dimension.y, ce.location.longitude, ce.location.latitude, ce.edge_end1, ce.edge_end2);
